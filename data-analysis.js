...
function daPrecalcularBumpChart(players, finishedMatches) {
  ...
  let todayCEST = daGetTodayCEST();
  if (todayCEST > TOURNAMENT_END) {
    todayCEST = TOURNAMENT_END;
  }
...
/* -----------------------------------------------------------
   ...
   ----------------------------------------------------------- */
function getTeamRoundFromData(team, data, faseGruposTerminada) {
  ...
  for (const round of rounds) {
    ...
    for (const m of matches) {
      if (m.team1 === team || m.team2 === team) {
        // Si el partido ya tiene ganador
        if (m.winner) {
          if (m.winner === team) {
            // Ganador
          ...
          // Partido programado pero aún sin jugar → el equipo ha alcanzado esta ronda
          return round;
        }
      }
    }
  ...
  // Si no aparece en eliminatorias pero la fase de grupos terminó, asignar ronda según clasificación
...
function daCalculateScore(player, real, faseGruposTerminada) {
  ...
  Object.keys(real.groups || {}).forEach(g => {
    (real.groups[g] || []).forEach(t => allTeams.add(t));
  });
  (real.thirdPlace || []).forEach(t => allTeams.add(t));
  ...
  allTeams.forEach(team => {
    const predRound = getTeamRoundFromData(team, player, true);
    const realRound = getTeamRoundFromData(team, real, realFaseGruposTerminada);
...
function daCalculateScoreByCategory(player, real, faseGruposTerminada) {
  const cats = {
    ...
    groupPositions: 0,
    bestThirds: 0,
    knockoutRounds: 0,   // rondas round32 a semifinals
    finalPositions: 0     // champion, finalist, thirdPlace, fourthPlace
  };
  ...
  Object.keys(groups).forEach(g => {
    (groups[g] || []).forEach(t => allTeams.add(t));
  });
  (player.thirdPlace || []).forEach(t => allTeams.add(t));

  allTeams.forEach(team => {
    const predTerminal = getRound(team, player);
    const realTerminal = getRound(team, real);
    if (!predTerminal || !realTerminal) return;

    const realRounds = roundsFor(realTerminal);
    const predRounds = roundsFor(predTerminal);
